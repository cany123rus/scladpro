...
## Удалённые разделы (2026-06)
- Разделы **Товары (`products`)**, **Instagram (`instagram`)**, **Удаление/Корзина (`trash`)** убраны:
  из `DASHBOARD_TAB_IDS`, обоих меню (`menuItems`, `EMPLOYEE_PERMISSION_SECTIONS`), удалены JSX-блоки
  и мёртвые `useEffect`-триггеры. UI недостижим.
- ⚠️ Таблица `products` в БД и `from('products')` НЕ трогать — используется поставками/импортом.
- Остались как мёртвый код (не вызываются из UI, но переплетены с realtime/импортом): `fetchProducts`,
  `fetchTrashItems`, trash-машинерия, instagram-хендлеры/стейты. Удалять — отдельным аккуратным рефакторингом.
