  ...
  return (
    <div className="max-w-full mx-auto px-4">
      <div className="mb-6">
        ...
        <p className="text-slate-500 mt-1">Потоки в реальном времени. Для вывода в браузере используй HTTP/HLS/WebRTC URL. RTSP можно открыть во внешнем плеере.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {cameraStreams.map((cam, idx) => {
          ...
          return (
            <div key={`cam-${idx}`} className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm">
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-50 overflow-hidden min-h-[220px] flex items-center justify-center">
                {isHttp && cam.url.includes('/stream.html') ? (
                  ...
                  <div className="p-4 text-sm text-slate-500 text-center">Укажи URL потока, чтобы увидеть видео.</div>
                )}
              </div>
