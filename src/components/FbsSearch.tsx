...
/*
 * Про адреса коробок и почему их нельзя угадывать по виду значения.
 *
 * Была проверка «адрес должен выглядеть как WB_1553573331, короткие числа —
 * это счётчик». Она отвергала настоящие файлы: в возвратах Власенко коробки
 * пронумерованы просто 1…72, и это законные адреса.
 *
 * Разделяет файлы не форма значения, а НАЗВАНИЕ столбца: у терминала он
  ...
  async function handleFiles(files: FileList | null, expected: 'stock' | 'picking') {
    ...
    try {
      ...
      for (const file of list) {
        ...
        /*
         * Без столбца адреса файл не берём: искать было бы негде, а молча
         * принятая поставка потом раскладывает лист по пустоте. Именно так
         * сюда попала сводка по артикулам — у неё адреса нет вовсе.
         */
        if (!cBox) {
          showToast(
            `${file.name}: нет столбца «Адрес» с номером коробки — это не выгрузка терминала`,
            'error',
          );
          continue;
        }
