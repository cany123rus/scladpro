...
Deno.serve(async (req) => {
  ...
  try {
    switch (action) {
      ...
      /*
       * Разведка по типам документов убрана намеренно.
       *
       * 09.09.2026 я проверял допустимые типы, отправляя в боевой контур тело с
       * пустым документом и заведомо неверной подписью — в расчёте, что провал
       * проверки произойдёт сразу. Расчёт был неверный: ГИС МТ ответил 201 и
       * завёл четыре документа, которые потом отвалились на разборе. Кодов они
       * не касались, но в кабинете организации остались.
       *
       * Вывод на будущее: боевой контур — не место для проб. Разведка только
       * чтением (/cises/info) или в песочнице с тестовым сертификатом.
       */
      ...
      case 'poll': {
        ...
        for (const doc of docs || []) {
          ...
          /*
           * Адрес проверки статуса нужно уточнить.
           *
           * `/doc/{id}/info` в этой версии true-api отвечает 410 «Устаревшее
           * API», а `/doc/list` и остальные кандидаты — 404. Пока рабочего
           * адреса нет, статус документа смотрят в личном кабинете ГИС МТ;
           * здесь мы честно ничего не меняем, вместо того чтобы записывать
